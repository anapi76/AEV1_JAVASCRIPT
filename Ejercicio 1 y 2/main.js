// creo un array con los datos de las 3 series
let seriesList = [
...
let filmsButton = document.getElementsByClassName('nutrition')[1];

// Selecciono los botones anterior y siguiente interio
...
function select(list) {
    titleH1.innerHTML = list[0].title;
    img.src = list[0].img;
    char.innerHTML = list[0].characters[0].name;
    act.innerHTML = list[0].characters[0].actor;
    currentList = list;
    indexCurrentOut = 0;
}
...
function previousOut() {
    ...
    return currentOut();//Llama a la función que muestra los resultados
}
...
// Función para mostrar por pantalla el resultado de clickar en el botón anterior y siguiente exterior
function currentOut() {
    titleH1.innerHTML = currentList[indexCurrentOut].title;
    img.src = currentList[indexCurrentOut].img;
    char.innerHTML = currentList[indexCurrentOut].characters[0].name;
    act.innerHTML = currentList[indexCurrentOut].characters[0].actor;
}
...
// Función para mostrar por pantalla el resultado de clickar en el botón anterior y siguiente interior
function currentIn() {
...
seriesButton.addEventListener('click', () => select(seriesList));
filmsButton.addEventListener('click', () => select(filmsList));
